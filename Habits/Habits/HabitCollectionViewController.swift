...
class HabitCollectionViewController: UICollectionViewController {
        ...
        print(model.habitsByName)
            var itemsBySection = model.habitsByName.values.reduce(
                ...
                if model.favoriteHabits.contains(habit) {
                    ...
                    section = .category(habit.categoty)
    ...
    func createDataSource() -> DataSourceType {
        ...
        dataSource.supplementaryViewProvider = { (collectionView, kind,
           indexPath) in
            let header =
               collectionView.dequeueReusableSupplementaryView(ofKind: sectionHeaderKind, withReuseIdentifier: sectionHeaderIdentifier, for: indexPath) as! NamedSectionHeaderView
    ...
    override func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        let config = UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { _ in
            ...
            let favoriteToggle = UIAction(title: item.isFavorite ? "Unfavorite" : "Favorite") { (action) in Settings.shared.toggleFavorite(item.habit)
                self.updateCollectionView()
            }
